...
export async function GET() {
  ...
  const { data } = await supabase.from("employees").select("id, full_name, email").eq("is_cio", true).order("full_name");
  return NextResponse.json({ approvers: data ?? [] });
}

// body: { email: string, is_cio: boolean }
export async function POST(request: NextRequest) {
  const supabase = await createClient();
  const result = await requireAdmin(supabase);
  if ("error" in result) return result.error;

  const { email, is_cio } = await request.json();
  ...
  const { error } = await admin.from("employees").update({ is_cio: !!is_cio }).eq("id", employee.id);
