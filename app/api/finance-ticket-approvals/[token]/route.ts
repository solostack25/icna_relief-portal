...
export async function GET(_req: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: ticket } = await admin
    ...
    .select("ticket_number, title, category, total, status, submitted_at, employees:requestor_id(first_name, last_name)")
  ...
  return NextResponse.json({ step, ticket, priorSteps });
}
