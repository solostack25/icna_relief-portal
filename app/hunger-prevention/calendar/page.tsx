...
import HungerCalendarClient from "./HungerCalendarClient";

export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ office?: string }> }) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <p className="text-sm mb-6" style={{ color: "rgba(22,48,43,0.5)" }}>
        Set pickup windows, repeat them weekly, or block off a day entirely (holidays, closures).
      </p>
      {officeId ? (
        <HungerCalendarClient officeId={officeId} />
      ) : (
        <p className="text-sm" style={{ color: "rgba(22,48,43,0.5)" }}>
          {access.isAdmin ? "Pick an office above to manage its calendar." : "No office is assigned to your account yet."}
        </p>
      )}
    </div>
  );
