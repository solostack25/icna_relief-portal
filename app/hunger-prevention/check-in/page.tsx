...
export default async function CheckInPage({ searchParams }: { searchParams: Promise<{ office?: string }> }) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      {officeId ? (
        <CheckInClient officeId={officeId} />
      ) : (
        <p className="text-sm" style={{ color: "rgba(22,48,43,0.5)" }}>
          {access.isAdmin ? "Pick an office above to see today's check-in list." : "No office is assigned to your account yet."}
        </p>
      )}
    </div>
  );
