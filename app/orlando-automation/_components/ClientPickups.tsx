...
const STATUS_KEYS: Record<string, string> = {
  booked: "pickups.status.booked",
  completed: "pickups.status.completed",
  missed: "pickups.status.missed",
  expired: "pickups.status.expired",
  cancelled: "pickups.status.cancelled",
};

export default function ClientPickups({ clientId }: { clientId: string }) {
  ...
  async function loadBookings() {
    ...
    const { data: slotRows } = slotIds.length
      ? await supabase
          .from("pickup_slots")
          .select("id, slot_date, start_time, end_time, office")
          .in("id", slotIds)
      : { data: [] as Slot[] };
  ...
  async function loadAvailableSlots() {
    ...
    const { data: slotRows } = await supabase
      .from("pickup_slots")
      .select("id, slot_date, start_time, end_time, office, capacity")
  ...
  async function handleBook(slotId: string) {
    ...
    const graceEnds = slot
      ? new Date(new Date(slot.slot_date + "T00:00:00").getTime() + 3 * 24 * 60 * 60 * 1000)
          .toISOString()
          .slice(0, 10)
      : null;
    ...
    await logAudit(supabase, employeeId, "staff_book_pickup", "pickup_booking", null, {
      client_id: clientId,
      slot_id: slotId,
    });

    showAlert(t("pickups.bookedSuccess"), "success");
    setShowBooker(false);
    loadBookings();
  }
  ...
  async function handleCancel(bookingId: string) {
    ...
    await logAudit(supabase, employeeId, "staff_cancel_pickup", "pickup_booking", bookingId, {
      client_id: clientId,
    });

    showAlert(t("pickups.cancelledSuccess"), "success");
    loadBookings();
  }

  // Warn (but don't block) if this client already has a booking this calendar month.
  ...
  return (
    <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden mb-6">
      <div className="flex items-center justify-between px-6 pt-6 pb-4">
        <h2 className="text-sm font-medium">{t("pickups.title")}</h2>
        <button
        ...
        >
          {showBooker ? t("pickups.cancel") : t("pickups.bookPickup")}
        </button>
      </div>

      {showBooker && (
        <div className="mx-6 mb-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-input-bg)] p-4">
          <p className="text-xs text-[var(--color-text-dim)] mb-3">{t("pickups.overrideNote")}</p>
          {hasBookingThisMonth && (
            <p className="text-xs text-amber-700 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 rounded-lg px-3 py-2 mb-3">
              {t("pickups.alreadyThisMonth")}
            </p>
          )}
          {loadingSlots ? (
            <p className="text-xs text-[var(--color-text-dim)]">{t("pickups.loadingTimes")}</p>
          ) : availByDate.size === 0 ? (
            <p className="text-xs text-[var(--color-text-dim)]">{t("pickups.noSlots")}</p>
          ) : (
                    ...
                    {daySlots.map((s) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={remaining <= 0 ? "text-red-600" : "text-[var(--color-text-dim)]"}
                          >
                            {booking === s.id
                              ? t("pickups.booking")
                              : remaining <= 0
                                ? t("pickups.full")
                                : t("pickups.spotsLeft", { n: remaining })}
                          </span>
                        </button>
                      );
                    })}
      ...
      {loading ? (
        <p className="px-6 pb-6 text-sm text-[var(--color-text-dim)]">{t("common.loading")}</p>
      ) : bookings.length === 0 ? (
        <p className="px-6 pb-6 text-sm text-[var(--color-text-dim)]">{t("pickups.none")}</p>
      ) : (
        <div>
          {bookings.map((b) => {
            const slot = slots.get(b.slot_id);
              ...
              <div key={b.id} className="px-6 py-3 border-t border-[var(--color-border)]">
                <div className="flex items-center justify-between">
                  <div className="text-sm">
                    {slot
                      ...
                      : t("pickups.unknownSlot")}
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={`text-xs font-medium ${statusColor[b.status] ?? ""}`}>
                      {t(STATUS_KEYS[b.status] ?? "pickups.status.booked")}
                    </span>
                ...
                {confirming && (
                  <div className="mt-2 flex items-center gap-2 rounded-lg bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 px-3 py-2">
                    ...
                    >
                      {cancelling === b.id ? t("pickups.cancelling") : t("pickups.yesCancel")}
                    </button>
                    <button
